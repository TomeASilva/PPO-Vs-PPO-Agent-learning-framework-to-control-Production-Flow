...
class Agent:
    def __init__(self,
                 ...
                 fixed_ppo_networks_configuration,
                 gamma,
        ...
        self.fixed_ppo_networks_configuration = fixed_ppo_networks_configuration
        self.n_reward_returns = n_reward_returns 
        
    def collect_episodes_evaluation(self, 
                                    number_ep,
                                    files,
                                    logging):
        ...
        for ep in range(number_ep):
            ...
            self.production_system_1 = ProductionSystem(env=env,
                             **self.production_system_configuration,
                             ...
                             policy=self.con_wip_model, # will be defined only after instatiation of child classes
            ...
            self.production_system_3 = ProductionSystem(env=env,
                                                   **production_system_configuration,
                                                   ep_buffer=self.buffer3,
                                                   twin_system=self.production_system_1,
        ...
        if files:
            return (self.production_system_1.path, self.production_system_2.path, self.production_system_3.path)
    ...
    def training_loop(self):
        try:
            ...
            if self.save_checkpoints:
                ...
                #Load models for the fixed Policy
            ...
            while self.current_number_episodes.value < self.total_number_episodes:
                ...
                if self.number_optimization_cycles % 1 == 0:
                    rewards_volatile = []
                for i in range (1):
                    path_conwip, path_PPO, path_fixed = self.collect_episodes_evaluation(1, True, False)
                    reward_PPO = self.production_system_2.sum_rewards 
                    reward_fixed = self.production_system_3.sum_rewards 
        ...
        while self.current_number_episodes.value < self.total_number_episodes:
            ...
            for ep in range(self.number_episodes_worker): # Run more than episode per iteration of PPO
                if self.current_number_episodes.value < self.total_number_episodes:
                    self.collect_episodes_training (self.number_episodes_worker, False, False)
